...
$sql="update  tbl_bookdetails set BOOK_IMG=:imgnewname where BOOK_ID=:bookid";
...
<?php 
$bookid=intval($_GET['bookid']);
$sql = "SELECT tbl_bookdetails.BOOK_TITLE,tbl_bookdetails.BOOK_ID as bookid,tbl_bookdetails.BOOK_IMG from  tbl_bookdetails  where tbl_bookdetails.BOOK_ID=:bookid";
$query = $dbh -> prepare($sql);
$query->bindParam(':bookid',$bookid,PDO::PARAM_STR);
$query->execute();
...
{               ?>  
<input type="hidden" name="curremtimage" value="<?php echo htmlentities($result->BOOK_IMG);?>">
<div class="col-md-6">
...
<input class="form-control" type="text" name="bookname" value="<?php echo htmlentities($result->BOOK_IMG);?>" readonly />
</div></div>
